...
execute if score @s ghost.boss.hurt matches 0 run return fail


playsound item.shield.break master @a ~ ~ ~ 1 1.5

execute on vehicle if score @s ghost.boss.hp.temp matches ..0 run function boss:tick/boss/bow/die
